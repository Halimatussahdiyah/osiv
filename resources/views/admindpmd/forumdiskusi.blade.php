...
<div class="card">
  ...
  <div class="card-body">
    ...
    <div class="container">
      ...
      <span class="right">{{ $f->judul_forum }}({{ $f->pengirim }}) <a
          href="/admindpmd/tanggapi/{{ $f->id_forum}}">Tanggapi</a></span>
      @else
      {{-- ini buat admin desa --}}
      <p>{{ $f->deskripsi }}</p>
      <img src="{{ asset('foto_kegiatan/'.$f->foto) }}" alt="" data-toggle="modal"
        data-target="#exampleModal{{ $f->id_forum }}">
      <span class="left">{{ $f->judul_forum }}({{ $f->pengirim }}) <a
          href="/admindpmd/tanggapi/{{ $f->id_forum}}">Tanggapi</a></span>
      @endif
    </div>
    ...
    <div class="row justify-content-end">
      <div class="col-12">
        <div class="container">
          ...
          <span class="right">{{ $f->judul_forum }}({{ $tanggapi->pengirim }})</span>
          <a href="/admindpmd/tanggapi/{{ $f->id_forum}}">Tanggapi</a>
        </div>
      </div>
    </div>
...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="modal-body">
          <form action="{{route('forum_diskusi.store')}}" class="form-horizontal tasi-form" method="post"
            enctype="multipart/form-data">
            @csrf
            <div class="form-group">
              <label for="recipient-name" class="col-form-label">Pengirim:</label>
              <input type="text" class="form-control" id="recipient-name" name="pengirim" value="Admin DPMD" readonly>
            </div>
            <div class="form-group">
